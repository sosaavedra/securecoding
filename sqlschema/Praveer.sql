...
CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteRejectedClient`(IN `in_client_id` INT(8)) NOT DETERMINISTIC CONTAINS SQL SQL SECURITY DEFINER BEGIN DELETE from client where id=in_client_id; DELETE from user where person_id=in_client_id; END
